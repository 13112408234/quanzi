<template>
	<view class="blog" @click="goDetail">
		...
		<view class="sbian">
			<view class="zb">
				<view class="tx"><u-avatar :src="UserAvatar(item)" shape="circle" size="20"></u-avatar></view>
				<view class="yhm">{{giveName(item)}}</view>
				<view class="time">
		...
		<view class="zhojian">
			...
			<view class="msu" v-if="item.description">{{item.description}}</view>
			<view class="_image" v-if="item.picurls">
		...
		<view class="xiamian">
			...
			<view :class="(item.likeState || '')?'active':''">
				<text class="iconfont icon-a-106-xihuan" @click.stop="likeClick(index)"></text>
				<text>{{item.like_count && item.like_count > 0?item.like_count:'点赞'}}</text>
			</view>
		</view>
		<!-- 点赞评论收藏结束 -->
	</view>
	...
	export default {
		...
		methods: {
			...
			goDetail(e) {
				...
				//把id传给详情页
				uni.navigateTo({
					url: '/pages/detail/detail?id=' + this.item._id + '&index=' + this.index
				})
			},
			//在列表点击预览图片
			djimg(index, e) {
